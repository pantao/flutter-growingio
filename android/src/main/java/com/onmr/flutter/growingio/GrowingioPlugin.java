package com.onmr.flutter.growingio;
...
/** GrowingioPlugin */
public class GrowingioPlugin implements MethodCallHandler {
  /** Plugin registration. */
  public static void registerWith(Registrar registrar) {
    final MethodChannel channel = new MethodChannel(registrar.messenger(), "growingio");
    channel.setMethodCallHandler(new GrowingioPlugin());
  }
  ...
  private void onTrack(MethodCall call){
    ...
    Double number = call.argument("number");
    boolean hasNumber = call.hasArgument("number");
    GrowingIO gio = GrowingIO.getInstance();
    if (call.hasArgument("variable")){
      JSONObject variable = toJson(call.argument("variable"));
      if (variable == null) return;
      if (hasNumber){
        gio.track(eventId, number, variable);
      }
      else {
